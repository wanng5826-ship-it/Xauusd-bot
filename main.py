"""
=======================================================
  Price Action Multi-Pair Bot  —  v15.0
  Pairs    : 19 pair forex + komoditas + BTCUSD
  Strategy : Price Action SEMUA PAIR (Forex + BTC sama)
  AI       : Groq Llama3 (analisis makro)
  Data     : yfinance + FRED API + NewsAPI
  Notif    : Telegram

  CHANGELOG v15.0 — H4 + BTC SAMA STRATEGI FOREX
  [NEW] Tambah timeframe H4 sebagai konfirmasi trend utama
        Alur: H4 trend → H1 sinyal PA → 15M konfirmasi
  [NEW] BTC pakai strategi Price Action (sama seperti forex)
        Bukan lagi News Sentiment — sekarang konsisten semua pair
  [NEW] detect_swing_structure dipakai di H4 untuk trend mayor
  [NEW] scan_price_action tetap di H1, konfirmasi tetap 15M
  [NEW] SCALPING_PAIRS dihapus — semua pair pakai analyze_pair
  [NEW] BTC pip_config ditambahkan ($50 pip, SL 300, TP 600)
  [FIX] Signal message format seragam semua pair

  CHANGELOG v14.0:
  [OLD] Price Action H1 + 15M konfirmasi (forex)
  [OLD] BTC News Sentiment (diganti v15.0)
=======================================================
"""
...
PIP_CONFIG = {
    ...
    "BTCUSD" : {"pip_size": 1.0,    "sl_pip": 300,"tp_pip": 600},
}
...
def init_db():
    ...
    try:
        ...
        # Fix kolom structure yang terlalu pendek (VARCHAR(20) → VARCHAR(60))
        try:
            cur.execute("ALTER TABLE signal_history ALTER COLUMN structure TYPE VARCHAR(60)")
        except Exception:
            pass  # Sudah benar atau tabel baru
...
def scan_price_action_h4(df_4h, df_1h, df_15m, pair, trend_4h):
    """
    Versi H4-aware dari scan_price_action.
    Trend mayor dari H4 dipakai sebagai filter utama:
    - UPTREND H4   → hanya izinkan sinyal BUY di H1
    - DOWNTREND H4 → hanya izinkan sinyal SELL di H1
    - SIDEWAYS H4  → izinkan keduanya (seperti sebelumnya)
    Selanjutnya konfirmasi 15M tetap sama.
    """
    if len(df_1h) < 50 or len(df_15m) < 20:
        return None

    trend_1h      = detect_swing_structure(df_1h, lookback=40)
    sr_levels     = detect_support_resistance(df_1h, lookback=50)
    ...
    # ── 1. Pin Bar ────────────────────────────────
    pin = detect_pin_bar(curr_1h, trend_1h)
    if pin == "BUY" and (near_support or trend_1h == "UPTREND"):
        results.append(("BUY", "Pin Bar (Hammer)", current_price))
    if pin == "SELL" and (near_resistance or trend_1h == "DOWNTREND"):
        results.append(("SELL", "Pin Bar (Shooting Star)", current_price))

    # ── 2. Engulfing ──────────────────────────────
    eng = detect_engulfing(curr_1h, prev_1h, trend_1h)
    if eng == "BUY" and (near_support or trend_1h == "UPTREND"):
        results.append(("BUY", "Bullish Engulfing", current_price))
    if eng == "SELL" and (near_resistance or trend_1h == "DOWNTREND"):
        results.append(("SELL", "Bearish Engulfing", current_price))

    # ── 3. Inside Bar Breakout ─────────────────────
    ib_action, ib_level = detect_inside_bar_breakout(df_1h)
    if ib_action == "BUY" and trend_1h in ("UPTREND", "SIDEWAYS"):
        results.append(("BUY",  "Inside Bar Breakout", ib_level or current_price))
    if ib_action == "SELL" and trend_1h in ("DOWNTREND", "SIDEWAYS"):
        results.append(("SELL", "Inside Bar Breakout", ib_level or current_price))

    if not results:
        print(f"[{pair}] Tidak ada sinyal Price Action H1 → NO TRADE")
        return None

    # ── Filter berdasarkan trend H4 ───────────────
    if trend_4h == "UPTREND":
        results = [r for r in results if r[0] == "BUY"]
        if not results:
            print(f"[{pair}] Trend H4 UPTREND → sinyal SELL difilter")
            return None
    elif trend_4h == "DOWNTREND":
        results = [r for r in results if r[0] == "SELL"]
        if not results:
            print(f"[{pair}] Trend H4 DOWNTREND → sinyal BUY difilter")
            return None
    # SIDEWAYS H4 → semua sinyal boleh lanjut

    action, signal_name, key_level = results[0]
    print(f"[{pair}] PA: {signal_name} → {action} | H4:{trend_4h} H1:{trend_1h} | Level:{round(key_level,5)}")
    ...
    return action, signal_name, round(key_level, 5), trend_1h
...
def analyze_pair(pair, symbol):
    print(f"\n{'='*10} [{pair}] {'='*10}")
    print(f"[{pair}] Menganalisis (Price Action H4+H1+15M)...")

    df_4h  = get_data(symbol, "4h",  "90d")
    df_1h  = get_data(symbol, "1h",  "60d")
    df_15m = get_data(symbol, "15m", "10d")

    if df_4h is None or len(df_4h) < 30:
        print(f"[{pair}] Data 4H tidak cukup, skip")
        return None
    ...
    # ── Trend Mayor dari H4 ────────────────────────
    trend_4h = detect_swing_structure(df_4h, lookback=40)
    print(f"[{pair}] Trend H4 (mayor): {trend_4h}")

    # ── Scan sinyal PA di H1 dengan filter trend H4 ──
    pa_result = scan_price_action_h4(df_4h, df_1h, df_15m, pair, trend_4h)
    if pa_result is None:
        return None

    action, signal_name, key_level, trend_1h = pa_result
    ...
    print(f"[{pair}] ✅ {action} | {signal_name} | H4:{trend_4h} H1:{trend_1h} | Entry:{entry} SL:{sl} TP:{tp} RR:1:{rr}")
    ...
    return {
        ...
        "tp"           : tp,
        "rr"           : rr,
        "structure"    : f"{trend_4h} (H4) / {trend_1h} (H1)",
...
def main():
    print("=" * 55)
    print("  Price Action Bot  v15.0  [Railway]")
    ...
    send_telegram(
        "🤖 <b>Price Action Bot v15.0 — ONLINE!</b>\n"
        "━━━━━━━━━━━━━━━━━━━━━━━\n"
        f"📊 Pairs       : {len(PAIRS)} pair aktif\n"
        "📈 Strategy    : Price Action SEMUA PAIR (Forex + BTC)\n"
        "📐 Trend Mayor : Swing Structure H4 → filter sinyal H1\n"
        "📐 Sinyal      : Pin Bar, Engulfing, Inside Bar (H1)\n"
        "✅ Konfirmasi  : RSI + Candle 15M\n"
    ...
    while True:
        ...
        for pair, symbol in PAIRS.items():
            ...
            try:
                ...
                trend_emj = "📈" if "UPTREND" in result.get("structure", "") else "📉"
                ...
                msg = (
                    ...
                    f"━━━━━━━━━━━━━━━━━━━━━━━\n"
                    f"{trend_emj} <b>Trend H4/H1       :</b> {result['structure']}\n"
