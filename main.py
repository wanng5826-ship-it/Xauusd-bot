...
=======================================================
  ...
  Strategy : Price Action (SEMUA pair, termasuk BTC)
  AI       : Groq Llama3 (analisis makro ekonomi)
  Data     : yfinance + FRED API + NewsAPI
  Notif    : Telegram

  CHANGELOG v15.0 — BTC GANTI KE PRICE ACTION:
  [NEW] BTCUSD kini pakai strategi Price Action
        sama persis dengan pair forex & komoditas
  [NEW] Sinyal BTC: Pin Bar, Engulfing, Inside Bar
  [NEW] Trend BTC dari Swing Structure H1 (HH/HL & LH/LL)
  [NEW] SL/TP BTC berbasis ATR + swing level
  [REM] Hapus strategi News Sentiment khusus BTC
  [REM] Hapus CryptoPanic, RSS feed, analisis sentimen BTC
  [UNI] 1 fungsi analyze_pair untuk semua 19 pair

  CHANGELOG v14.0:
  [OLD] Price Action (forex) + News Sentiment (BTC)
...
PIP_CONFIG = {
    ...
    "BTCUSD" : {"pip_size": 1.0,    "sl_pip": 500,"tp_pip": 1000},
}
...
def check_open_trades():
    ...
    try:
        ...
        for trade in trades:
            ...
            # Ambil harga terkini — pakai High/Low agar tidak miss spike SL/TP
            try:
                ticker      = yf.Ticker(symbol)
                df_tmp      = ticker.history(period="1d", interval="5m")
                if df_tmp is None or len(df_tmp) < 1:
                    continue
                last_candles  = df_tmp.tail(3)
...
def send_wr_report(period_days=7):
    ...
    msg = (
        ...
        f"{detail_text}\n"
        f"━━━━━━━━━━━━━━━━━━━━━━━\n"
        f"🤖 Price Action Bot v15.0 | Data: PostgreSQL Railway"
    )
    send_telegram(msg)
...
def get_news(pair):
    keywords = {
        ...
        "BTCUSD": "Bitcoin BTC crypto Fed interest rate",
    }
...
def main():
    ...
    send_telegram(
        ...
        "📈 Strategy    : Price Action (SEMUA pair termasuk BTC)\n"
        "📐 Sinyal      : Pin Bar, Engulfing, Inside Bar Breakout\n"
