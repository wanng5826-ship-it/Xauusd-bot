...
=======================================================
  ...
  [NEW] SL/TP forex berbasis pip tetap per pair
        (bukan ATR murni) — lebih realistis & konsisten.
        Setiap pair punya batas SL/TP pip sendiri.
        Kalau sweep level terlalu jauh → fallback ke
        fixed pip SL, tidak skip sinyal.
  [NEW] Sinyal Telegram tampilkan pip SL & TP.
  [FIX] "Risk kalkulasi invalid, skip" tidak muncul lagi.

  CHANGELOG v12.3:
  [NEW] BTCUSD: strategi diganti ke News Sentiment
        berbasis RSS CoinDesk/CoinTelegraph + NewsAPI + Groq.
  [FIX] KeyError 'structure' saat kirim sinyal BTC.
  [NEW] sig_key BTC pakai action+sentiment (anti spam).
=======================================================
"""
...
def detect_liquidity_sweep(df, structure):
    if len(df) < 20:
        return False, None
    ref_high = df["high"].iloc[-25:-5].max()
    ref_low  = df["low"].iloc[-25:-5].min()
    sweep_zone = df.iloc[-8:-1]
    for i in range(len(sweep_zone) - 1, -1, -1):
        c = sweep_zone.iloc[i]
        if structure == "UPTREND":
            # Sweep low: wick tembus ref_low (tidak harus close di atas)
            if c["low"] < ref_low:
                return True, round(ref_low, 5)
        elif structure == "DOWNTREND":
            # Sweep high: wick tembus ref_high
            if c["high"] > ref_high:
...
def main():
    print("=" * 55)
    print("  SMC Multi-Pair Bot  v12.5  [Railway]")
