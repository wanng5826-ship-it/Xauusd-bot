...
=======================================================
  ...
  CHANGELOG v12.7:
  [NEW] WR Report — laporan winrate dikirim ke Telegram:
        • Otomatis setiap 24 jam
        • Manual via command: /wr (7 hari), /wr30 (30 hari),
          /wr1 (hari ini)
        • Isi: total sinyal, profit/loss, WR %, per pair,
          breakdown BTC vs Forex, streak 10 trade terakhir,
          visual bar winrate

  CHANGELOG v12.6:
  [FIX] detect_structure: majority voting 3 lookback.
  [FIX] detect_liquidity_sweep: zone diperluas.
  [FIX] analyze_pair: 4H SIDEWAYS tidak skip, fallback 1H.
...
def send_wr_report(period_days=7):
    ...
    msg = (
        ...
        f"  {streak_str}\n"
        f"━━━━━━━━━━━━━━━━━━━━━━━\n"
        f"🤖 SMC Bot v12.6 | Data: PostgreSQL Railway"
    )
    send_telegram(msg)
...
def main():
    print("=" * 55)
    print("  SMC Multi-Pair Bot  v12.7  [Railway]")
