...
def scan_patterns(df_1h, df_15m, pair):
    ...
    # BUY patterns (dibalik → SELL)
    ok, level, height = detect_double_bottom(df_1h)
    if ok: results.append(("SELL", "Double Bottom", level, height))

    ok, level, height = detect_inv_head_and_shoulders(df_1h)
    if ok: results.append(("SELL", "Inv Head & Shoulders", level, height))

    ok, level, height = detect_bull_flag(df_1h)
    if ok: results.append(("SELL", "Bull Flag", level, height))

    ok, level, height = detect_ascending_triangle(df_1h)
    if ok: results.append(("SELL", "Ascending Triangle", level, height))

    # SELL patterns (dibalik → BUY)
    ok, level, height = detect_double_top(df_1h)
    if ok: results.append(("BUY", "Double Top", level, height))

    ok, level, height = detect_head_and_shoulders(df_1h)
    if ok: results.append(("BUY", "Head & Shoulders", level, height))

    ok, level, height = detect_bear_flag(df_1h)
    if ok: results.append(("BUY", "Bear Flag", level, height))

    ok, level, height = detect_descending_triangle(df_1h)
    if ok: results.append(("BUY", "Descending Triangle", level, height))
    ...
    # Konfirmasi 15M: candle searah (dibalik)
...
def calc_sl_tp_pattern(df, action, breakout_level, pattern_height, pair):
    """SL/TP otomatis dari ukuran pattern, dengan minimum pip dari PIP_CONFIG."""
    cfg      = PIP_CONFIG.get(pair, {})
    pip_size = cfg.get("pip_size", 0.0001)
    min_sl   = cfg.get("sl_pip", 25) * pip_size   # SL minimum dari config
    min_tp   = cfg.get("tp_pip", 50) * pip_size   # TP minimum dari config
    price    = df["close"].iloc[-1]
    atr      = calc_atr(df)

    # SL: di belakang breakout level + buffer 1 ATR
    if action == "BUY":
        sl_raw = breakout_level - atr
        tp_raw = price + pattern_height
        # Pastikan SL tidak terlalu dekat
        if price - sl_raw < min_sl:
            sl_raw = price - min_sl
        if tp_raw - price < min_tp:
            tp_raw = price + min_tp
    else:
        sl_raw = breakout_level + atr
        tp_raw = price - pattern_height
        # Pastikan SL tidak terlalu dekat
        if sl_raw - price < min_sl:
            sl_raw = price + min_sl
        if price - tp_raw < min_tp:
            tp_raw = price - min_tp
